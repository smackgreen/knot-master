      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="budget">
          <div className="space-y-4">
            {/* Budget Header with Total */}
            <div className="flex justify-between items-center">
              <div className="flex items-baseline gap-2">
                <h2 className="text-2xl font-bold">{t('clients.budget')}: {client.budget ? formatCurrency(client.budget.totalBudget) : '0'} €</h2>
                <span className="text-muted-foreground text-sm">{client.budget ? formatCurrency(client.budget.totalBudget) : '0'} €</span>
              </div>
              <Button onClick={handleOpenUpdateBudget}>
                <CreditCard className="mr-2 h-4 w-4" />
                {t('clients.updateBudget')}
              </Button>
            </div>
            ...
            {client.budget ? (
                ...
                <div className="lg:col-span-8 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="border rounded-lg p-4 bg-gray-50">
                      <p className="text-muted-foreground text-sm">{t('clients.totalBudget')}</p>
                      <p className="text-xl font-bold mt-1">{formatCurrency(client.budget.totalBudget)}</p>
                    </div>
                  ...
                  {/* Main Budget Table */}
                  {client.budget.categories.length > 0 ? (
                    <div className="overflow-x-auto border rounded-lg">
                      <Table>
                        ...
                        <TableBody>
                          {client.budget.categories.map((cat, index) => {
                            const estimated = cat.allocated;
                            const categoryVendors = client.vendors?.filter(v => v.category === cat.category) || [];
                            const actual = categoryVendors.reduce((sum, vendor) => sum + (vendor.cost || 0), 0);
                            const difference = estimated - actual;
                            const paid = categoryVendors
                              .filter(v => v.isPaid)
                              .reduce((sum, vendor) => sum + (vendor.cost || 0), 0);
                            const remainderToPay = actual - paid;

                            return (
                              <Fragment key={index}>
                                <TableRow className="hover:bg-gray-50">
                                  <TableCell className="font-medium">{formatVendorCategory(cat.category)}</TableCell>
                                  ...
                                  <TableCell className={`text-right ${remainderToPay > 0 ? "text-red-600 font-medium" : ""}`}>
                                ...
                                {/* Optional: Add a detail row that can be expanded */}
                                {/* This would be shown when the ChevronDown button is clicked */}
                              </Fragment>
                            );
                          })}
                          ...
                          <TableRow className="font-medium bg-gray-50">
                            ...
                            <TableCell className="text-right">
                              {formatCurrency(client.budget.categories.reduce((sum, cat) => sum + cat.allocated, 0))}
                            </TableCell>
                            <TableCell className="text-right">
                              {formatCurrency(client.vendors?.reduce((sum, vendor) => sum + (vendor.cost || 0), 0) || 0)}
                            </TableCell>
                            <TableCell className="text-right">
                              {(() => {
                                const totalEstimated = client.budget.categories.reduce((sum, cat) => sum + cat.allocated, 0);
                                const totalActual = client.vendors?.reduce((sum, vendor) => sum + (vendor.cost || 0), 0) || 0;
                    ...
                    <div className="border rounded-lg p-6 text-center">
                      <p className="text-muted-foreground">{t('clients.noBudgetCategories')}</p>
                    </div>
                  )}
                </div>

                {/* Right side - Charts and Visualizations */}
                <div className="lg:col-span-4 space-y-4">
                  {/* Estimation Pie Chart */}
                  <div className="border rounded-lg p-4">
                    <h3 className="text-lg font-medium mb-4">{t('clients.estimation')}</h3>
                    ...
                    {client.budget.categories.length > 0 ? (
                      <div className="h-[250px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={client.budget.categories.map(cat => ({
                              ...
                              nameKey="name"
                            >
                              {client.budget.categories.map((entry, index) => {
                  ...
                  <div className="border rounded-lg p-4">
                    <h3 className="text-lg font-medium mb-4">{t('clients.budgetStatus')}</h3>
                          ...
                          return (
                            <div key={index} className="space-y-1">
                              <div className="flex justify-between text-xs">
                                ...
                                <span>{formatCurrency(actual)} / {formatCurrency(cat.allocated)}</span>
                              </div>
              ...
              <div className="border rounded-lg p-8 text-center">
                <p className="text-muted-foreground mb-4">{t('clients.noBudgetCreated')}</p>
                <Button onClick={handleOpenUpdateBudget}>{t('clients.createBudget')}</Button>
              </div>
            )}
          </div>
        </TabsContent>
